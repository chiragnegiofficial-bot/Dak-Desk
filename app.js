...
let pendingLedgerImport = { valid: [], errors: [] }, appPinHash = '';
...
function switchTab(name){
  ...
  if (name === 'operations') runOperationsScan();
}
...
async function initApp() {
    ...
    appPinHash = await localforage.getItem('appPinHashV1') || '';
    ...
    renderRegTable();
    renderDashboard();
    renderTable(); renderHistory(); updatePreview();
    renderPosbPresets(); updatePosbCalculator();
    checkBackupReminder();
    if(appPinHash) document.getElementById('appLockOverlay').classList.add('show');
}
...
window.syncTdFromLedger = async function() {
    let addedCount = 0; memAccReg.forEach(reg => { if(reg.scheme.includes('TD')) { const exists = memTdEntries.some(e => e.accNo === reg.acc); if(!exists) { let term = 1; if(reg.scheme.includes('2Y')) term = 2; if(reg.scheme.includes('3Y')) term = 3; if(reg.scheme.includes('5Y')) term = 5; const incentive = Math.round(reg.amt * (INCENTIVE_RATES[term] || 0)); const prNo = reg.prNo ? reg.prNo : getNextPrNo(); memTdEntries.push({ accNo: reg.acc, depName: reg.name, prNo: prNo, deposit: reg.amt, term: term.toString(), incentive: incentive }); addedCount++; } } });
    if(addedCount > 0) { await localforage.setItem('tdBillEntries', memTdEntries); renderTable(); updateRunningTotal(); showToast(`✅ Synced ${addedCount} TD accounts from Ledger!`); } else { showToast(`ℹ️ No new TD accounts to sync.`); }
}
...
async function updateHeaders(){ ['billMonth','bpmName','bpmAcc','spoName','hoName'].forEach(id => { let el = document.getElementById(id); if(el && document.getElementById('out-'+id)) document.getElementById('out-'+id).textContent = el.value || '__________'; }); const bd=document.getElementById('billDate')?.value; if(document.getElementById('out-billDate')) document.getElementById('out-billDate').innerHTML = bd ? getFormattedDate(bd) : '__________'; globalSpoName = document.getElementById('spoName').value; globalHoName = document.getElementById('hoName').value; let spoWrap = document.getElementById('out-spoNameWrapper'); if(spoWrap) spoWrap.textContent = globalSpoName ? ` via ${globalSpoName}` : ''; await localforage.setItem('tdBillSpo', globalSpoName); await localforage.setItem('tdBillHo', globalHoName); }
